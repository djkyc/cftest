...
# ====================================================
# VPS Cloudflare IPv4 优选 - ARMv7 版（自动运行）
# 支持 ARMv7（armv7l / 32-bit ARM）架构设备
#
# 功能（与 ARMv8 版一致）：
...
# 5) 保持原脚本功能：Token 验证、cfst 调用、创建 cf 快捷命令、每 4 小时定时任务
#
# 注意：
# - 需要你准备 ARMv7 版 cfst，可执行文件路径：/root/cf2/ipv4/cfst
...
CFST="$WORK_DIR/cfst"         # cfst 必须是 ARMv7 可执行文件
...
CF_ZONE_ID="YOUR_CLOUDFLARE_ZONE_ID"         # Zone ID
CF_RECORD_NAME="ip.eee.xx.kg"                # 目标 DNS 记录（A 记录）
...
get_ping_avg_latency_ms() {
  ...
  echo "$ping_output" | awk -F'/' '
    /rtt|round-trip/ { gsub(/[^0-9.\/]/,""); print $5; exit }
  ' 2>/dev/null || echo ""
}
...
main() {
  ...
  if [ ! -x "$CFST" ]; then
    log "❌ 未检测到 ARMv7 版 cfst，请放入：$CFST"
    log "   提示：chmod +x $CFST"
    return 1
  fi
